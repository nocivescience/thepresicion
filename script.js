...
ballsDisplay.forEach(({ x, y }) => {
  const ball = document.createElement("div");
  ball.setAttribute("class", "ballD");
  ball.style.cssText = `left: ${x}px; top: ${y}px; `;

  pantallaEl.appendChild(ball);
  ballsDisplayElements.push(ball);
});
...
wallsPantalla = [
  {columna:0, fila:0, horizontal:true, largo:20},
  {columna:0, fila:0, horizontal:false, largo:3},
  {columna:0, fila:3, horizontal:true, largo:20},
  {columna:20, fila:0, horizontal:false, largo:3},
  {columna:0, fila:1, horizontal:true, largo:20},
  {columna:7, fila:0, horizontal:false, largo:3},
  {columna:0, fila:2, horizontal:true, largo:20},
  {columna:13, fila:0, horizontal:false, largo:3},
].map((wall) => ({
  x: wall.columna * (pathW + wallW),
  y: wall.fila * (pathW + wallW),
  horizontal: wall.horizontal,
  length: wall.largo * (pathW + wallW)
}));

wallsPantalla.forEach(({ x, y, horizontal, length }) => {
...
window.addEventListener("mousemove", function (event) {
  if (gameInProgress) {
    ...
    mazeElement.style.cssText = `
        ...
        `;
    pantallaEl.style.cssText = `
      transform: rotateY(${rotationY}deg) rotateX(${-rotationX}deg) translateX(-50%)
    `;
    const gravity = 2;
...
function main2(timestamp){
  ...
  try{
    if(accelerationX !==undefined && accelerationY!==undefined){
      ...
      const frictionDeltaX = frictionX * timeElapsed;
      const frictionDeltaY = frictionY * timeElapsed;
      ballsDisplay.forEach((ball) => {
        ball.nextX = ball.x + ball.velocityX;
        ball.nextY = ball.y + ball.velocityY;
        wallsPantalla.forEach((wall, wi) => {
          if(wall.horizontal){
      ...
      ballsDisplay.forEach(({ x, y }, index) => {
        ballsDisplayElements[index].style.cssText = `left: ${x}px; top: ${y}px; `;
      });
    }
  }catch(error){
    throw error;
